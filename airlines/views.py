...
class SearchListView(View):
    ...
    def get(self, request):
            ...
            for i in trip_list:
                ...
                for name in range(number):
                    add = {
                        ...
                        # 'return_ticket' = airline.ticket_return.filter(fly_class=class_type[name])
                    }
                    print(add['fly_class'])
                    print(airline.ticket_return.all())
                    add['return_ticket'] = airline.ticket_return.filter(fly_class=add['fly_class'])
                    print(add['return_ticket'])
        ...
        print(f'fligh count is {flight_count}')
        print(trip_list_final)
        return render(
            request,
            self.template_name,
            {
